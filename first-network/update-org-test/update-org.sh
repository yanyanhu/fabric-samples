...
# Update the configtx.yaml to add new org, e.g. Org7, and then generate channel requreid artifacts which is Org7MSPanchors.tx here.
export ORG_MSP_NAME=Org7MSP
...
# Update docker-compose template to add peer0.org7 and also ca7 and run 'docker-compose -f ***.yaml up' or 'fabric.sh -m up' to update the deployment.

# Enter cli container to run peer CMD and perform the following operations:
# 1) Let peer0.org7 join mychannel
# 2) Update the anchor peer or Org7
# 3) Install chaincode in peer0.org7
